...
from DDAffinity.utils.protein.constants import chi_pi_periodic, AA
from DDAffinity.utils.misc import BlackHole
...
def get_optimizer(cfg, model):
    params_to_update = [
        {'params': [param for name, param in model.named_parameters() if 'esm_embed' not in name and 'foldx_ddg' not in name], 'lr': cfg.lr, 'initial_lr': cfg.lr, 'weight_decay': cfg.weight_decay},
        {'params': model.single_encoders[0].esm_embed.parameters(), 'lr': cfg.lr_2, 'initial_lr': cfg.lr_2,
         'weight_decay': cfg.weight_decay_2},
        {'params': model.single_encoders[1].esm_embed.parameters(), 'lr': cfg.lr_2, 'initial_lr': cfg.lr_2,
         'weight_decay': cfg.weight_decay_2},
        {'params': model.foldx_ddg.parameters(), 'lr': cfg.lr_3, 'initial_lr': cfg.lr_3,
         'weight_decay': cfg.weight_decay_3},
    ]
    if cfg.type == 'adam':
...
def warmup_CosineAnneal(warm_up_iters, T_iters, lr_max, lr_min, lr_2_max, lr_2_min, lr_3_max, lr_3_min):
    ...
    return [_lambda, _lambda2, _lambda2, _lambda3]
    # return [_lambda, _lambda3]
...
def load_model_from_checkpoint(ckpt_path, return_ckpt=False):
    from DDAffinity.models import get_model
...
class CrossValidation(object):
    def __init__(self, model_factory, config, num_cvfolds, early_stoppingdir='./early_stopping', logger=None):
        ...
        for model in self.models:
            ...
            early_stopping = EarlyStopping(early_stoppingdir, logger, patience=10, verbose=False)
