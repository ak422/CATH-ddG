...
from DDAffinity.utils.transforms.select_atom import chi_atom15_index_map,atom15to37_index_map
from DDAffinity.utils.protein.dihedral_chi import  atom37_to_torsion_angles, AA_TO_INDEX, ALL_ATOMS, ALL_ATOM_POSNS,ONE_TO_THREE, CHI_PI_PERIODIC_LIST
...
class AddAtomNoise(object):
    ...
    def __call__(self, data, dummy=None):
        ...
        # dihedral noise
        # _get_noise = lambda: ((torch.randn_like(output['dihedral'][:, 1]) * self.noise_backbone) * protein["phi_mask"])
        # output['dihedral'][:, 1] = output['dihedral'][:, 1] + _get_noise()
        # _get_noise = lambda: ((torch.randn_like(output['dihedral'][:, 2]) * self.noise_backbone) * protein["psi_mask"])
        # output['dihedral'][:, 2] = output['dihedral'][:, 2] + _get_noise()
        # _get_noise = lambda: ((torch.randn_like(output['dihedral'][:, 3:]) * self.noise_sidechain) * protein["chi_mask"])
        # output['dihedral'][:, 3:] = output['dihedral'][:, 3:] + _get_noise()
